...
//    }
    ...
    override suspend fun isFirstTimeLaunch(context: Context?, nowInMillis: Long): Boolean {
        ...
        val firstTime = sf.getBoolean(LinkAttributionConstants.Local.Prefers.FIRST_TIME_KEY, true)
        ...
        try {
            ...
            val storedInstallTime = sf.getLong(LinkAttributionConstants.Local.Prefers.INSTALL_TIME_KEY, 0L)
            if (storedInstallTime == 0L) {
                sf.edit().putLong(LinkAttributionConstants.Local.Prefers.INSTALL_TIME_KEY, installTimeMillis).apply()
            }
            ...
            if (timeDifferenceSeconds < 60) { // Adjust threshold as needed
                sf.edit().putBoolean(LinkAttributionConstants.Local.Prefers.FIRST_TIME_KEY, false).apply() // Mark as not first time
                return true
            } else {
                //If the time difference is greater than the threshold, and the app was reinstalled, it is not the first time
                sf.edit().putBoolean(LinkAttributionConstants.Local.Prefers.FIRST_TIME_KEY, false).apply()
                return false
            }
