package com.library.polar_gx.model.extension.file
...
fun File.logFile() {
    if (this.isDirectory) {
        PolarLogger.d("FILE_LOG", "logFile: dir = ${this.path}, size = ${this.listFiles()?.size}")
        this.listFiles()?.let {
            for (i in it.indices) {
                it[i].logFile()
            }
        ...
        PolarLogger.d("FILE_LOG", "logFile: file = ${this.path}")
    }
}
