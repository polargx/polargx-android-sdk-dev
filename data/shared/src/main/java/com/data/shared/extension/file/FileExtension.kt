package com.data.shared.extension.file
...
fun File.logFile() {
    if (this.isDirectory) {
        com.app.shared.logger.DebugLogger.d("FILE_LOG", "logFile: dir = ${this.path}, size = ${this.listFiles()?.size}")
        this.listFiles()?.let {
            for (i in it.indices) {
                it[i].logFile()
            }
        ...
        com.app.shared.logger.DebugLogger.d("FILE_LOG", "logFile: file = ${this.path}")
    }
}
